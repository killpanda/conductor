...
public class DataAccessorTest {
    ...
    public void setUp() {
        ...
        dataAccessor = new DataAccessor(executionDAO, indexDAO, objectMapper);
    }
    ...
    public void tesGetWorkflowById() throws Exception {
        ...
        InputStream stream = DataAccessorTest.class.getResourceAsStream("/test.json");
        byte[] bytes = IOUtils.toByteArray(stream);
        String jsonString = new String(bytes);
        when(indexDAO.get(any(), any())).thenReturn(jsonString);
        workflow = dataAccessor.getWorkflowById("workflowId", true);
        assertNotNull(workflow);
    ...
    public void testGetWorkflowsByCorrelationId() {
        ...
        List<Workflow> workflows = dataAccessor.getWorkflowsByCorrelationId("correlationId", true);
        ...
        workflows = dataAccessor.getWorkflowsByCorrelationId("correlationId", true);
        assertNotNull(workflows);
        assertEquals(1, workflows.size());
    }
    ...
    public void testRemoveWorkflow() {
        ...
        dataAccessor.removeWorkflow("workflowId", false);
    ...
    public void testArchiveWorkflow() throws Exception {
        ...
        dataAccessor.removeWorkflow("workflowId", true);
    ...
    public void testAddEventExecution() {
        ...
        boolean added = dataAccessor.addEventExecution(new EventExecution());
        assertFalse(added);
        verify(indexDAO, never()).addEventExecution(any());

        when(executionDAO.addEventExecution(any())).thenReturn(true);
        added = dataAccessor.addEventExecution(new EventExecution());
        assertTrue(added);
